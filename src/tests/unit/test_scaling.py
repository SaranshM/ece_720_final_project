...
class TestScaling(unittest.TestCase):
    ...
    def test_scaling(self):
        """Test scaling of float columns."""
        transformed = self.fe.fit_transform(self.data_imputed)

        # Adjust the tolerance for floating-point arithmetic precision issues.
        tolerance = 1e-6
        for col in self.fe.float_cols:
            column_min = transformed[col].min()
            column_max = transformed[col].max()
            self.assertTrue(column_min >= 0-tolerance and column_max <= 1+tolerance,
                            f"{col} not properly scaled. Min: {column_min}, Max: {column_max}")
